...
module.exports = {
    ...
    if (!message.guild) return;
        ...
        const url = `https://api.tenor.com/v1/random?q=anime_lick&key=D23GGWE67OM4`
        ...
        if (message.mentions.members.size !== 0) {
        async function ping() {
            const url = `https://api.tenor.com/v1/random?q=anime_lick`
        ...
        const embed = new Discord.MessageEmbed()
        .setColor('#202225')
        .setTitle(`${message.author.tag} licked ${member.user.tag}`)
        .setImage(`${result.results[0].media[0].gif.url}`)
